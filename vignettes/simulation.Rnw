...
The package provides two functions to simulate data from
compound models like \eqref{eq:definition-S}:
\begin{enumerate}
\item \code{rcompound} for standard, non hierarchical, compound models
  with, basically, one distribution for $N$ and one for $C$;
\item \code{simul} for models where both the frequency and the
  severity components can have a hierarchical structure and where, in
  addition, each observation may be assigned a weight (or volume).
\end{enumerate}

Function \code{rcompound} is faster and has a simpler interface for
standard compound models, but \code{simul} is much more flexible.

The next section briefly describes function \code{rcompound}. The rest
of the document focuses on the description and usage of \code{simul}.
\cite{Goulet:simpf:2008} discuss in more details the models
supported by \code{simul} and give more thorough examples.


\section{Simulation from standard compound models}
\label{sec:rcompound}

Function \code{rcompound} generate variates from the random variable
$S$ in \eqref{eq:definition-S} where the distribution of both random
variables $N$ and $C$ is non hierarchical. The function has three
arguments:
\begin{enumerate}
\item \code{n} for the number of variates to generate;
\item \code{model.freq} for the frequency model (random variable $N$);
\item \code{model.sev} for the severity model (random variable $C$).
\end{enumerate}

Arguments \code{model.freq} and \code{model.sev} are simple
\proglang{R} expressions consisting in calls to a random number
generation function with the number of variates omitted.

\begin{example}
  Let $S \sim \text{Compound Poisson}(1.5, G)$, where $1.5$ is the value
  of the Poisson parameter and $G$ is the cumulative distribution
  function of a gamma distribution with shape parameter $\alpha = 3$
  and rate parameter $\lambda = 2$. We obtain variates from $S$ using
  \code{rcompound} as follows:
<<echo=TRUE>>=
...
From now on, we consider simulation of data from hierarchical models.
We need a method to describe these models in \proglang{R} that will
