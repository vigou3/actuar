...
Package \pkg{actuar} provides functions that greatly facilitates
generation of random variates from various probability models commonly
used in actuarial applications. From the simplest to the most
sophicated, these functions are:
\begin{enumerate}
\item \code{rmixture} to simulate from discrete mixtures;
\item \code{rcompound} to simulate from compound models;
...
A random variable is said to be discrete mixture of the random
...
\end{equation}
where $p_1, \dots, p_n$ are probabilities such that
$\sum_{i = 1}^n p_i = 1$.
...
The specification of simulation models uses the syntax of \code{simul}
that is explained in greater detail in \autoref{sec:simul}. In a
nutshell, the models are expressed in a semi-symbolic fashion using an
object of mode \code{"expression"} where each element is a complete
...
\begin{example}
  ...
  \end{equation*}
  We generate $10$ random variates from this density using
  \code{rmixture} with the following expression.
<<echo=TRUE>>=
...
See also \autoref{ex:comppois} for a more involved usage or
\code{rmixture} to simulate from compound Poisson models.
...
distributed random variables of the claim amounts, and each is
independent of the frequency random variable $N$.

Function \code{rcompound} generates variates from the random variable
$S$ in \eqref{eq:definition-S} when the distribution of both random
variables $N$ and $C$ is non hierarchical (for the more general
hierarchical case, see \autoref{sec:simul}). The function has three
arguments:
\begin{enumerate}
\item \code{n} the number of variates to generate;
...
\begin{example}
  Let $S \sim \text{Compound Poisson}(1.5, G)$, where $1.5$ is the
  value of the Poisson parameter and $G$ is the cumulative
  distribution function of a gamma distribution with shape parameter
  $\alpha = 3$ and rate parameter $\lambda = 2$. We obtain variates
  from the random variable $S$ using \code{rcompound} as follows:
<<echo=TRUE>>=
...
\begin{example}
  \label{ex:comppois}
  By Theorem~9.7 of \cite{LossModels4e}, the sum of compound Poisson
  random variables is itself compound Poisson with Poisson parameter
  equal to the sum of the Poisson parameters and severity distribution
  equal to the mixture of the severity models.

  Let $S = S_1 + S_2 + S_3$, where $S_1$ is compound Poisson with
  $\lambda = 2$ and severity Gamma$(3, 1)$; $S_2$ is compound Poisson
  with $\lambda = 1$ and severity Gamma$(5, 4)$; $S_3$ is compound
  Poisson with $\lambda = 0.5$ and severity Lognormal$(2, 1)$. By the
  aforementioned theorem, $S$ is compound Poisson with
  $\lambda = 2 + 1 + 0.5 = 3.5$ and severity
  \begin{equation*}
    f(x) = \frac{4}{7}
    \left(
  ...
  Combining \code{rcompound} and \code{rmixture} we can generate variates
  of $S$ using one elegant expression as follows.
<<echo=TRUE>>=
x <- rcompound(1e5, rpois(3.5),
