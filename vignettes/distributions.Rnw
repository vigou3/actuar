%% NOTE: this file does not contain any R code. The .Rnw extension is
%% used to that the vignette is picked up by R CMD build.
...
\usepackage[utf8]{inputenc}
  ...
  \setlist[enumerate]{label=\roman*),labelsep=*,leftmargin=1.5em,align=left}
...
transformed beta and transformed gamma families; see
\autoref{sec:implementation} for details. The package also
...
incomplete gamma functions, respectively. Limited expected value
functions gain an extended range of admissible values by using
\code{expint}, \code{betaint} and \code{gammaint} at the \proglang{C}
level. Affected functions are:
\begin{enumerate}
\item \code{levtrbeta},
  \code{levgenpareto},
  \code{levburr},
  ...
  \code{levllogis},
  \code{levparalogis},
  \code{levinvparalogis} in the transformed beta family that use
  \code{betaint};
\item \code{levinvtrgamma},
  \code{levinvgamma},
  \code{levinvweibull} in the transformed gamma family that use
  \code{gammaint}.
\end{enumerate}
...
Functions \code{dinvgauss}, \code{pinvgauss} and \code{qinvgauss} rely
on \proglang{C} implementations of functions of the same name from
package \pkg{statmod} \citep{statmod}.
