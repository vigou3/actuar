...
\eqref{eq:betaint:2} and depends on package \pkg{expint}
\citep{expint} to compute the incomplete gamma function
\eqref{eq:gammainc} and the exponential integral \eqref{eq:expint} at
the C level. The routines are used to evaluate the limited expected
value for distributions of the transformed beta and transformed gamma
families.
