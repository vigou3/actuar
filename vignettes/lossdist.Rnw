...
\begin{enumerate}
\item support for 19 additional continuous distributions useful for
  loss severity modeling;
\item support for phase-type distributions useful in ruin
  theory;
\item support functions to compute raw moments, limited moments and
  the moment generating function (when it exists) of continuous
  distributions;
...
terms]{LossModels4e}. Table \ref{tab:coverage} summarizes the
...
Vignette \code{"coverage"} contains more detailed pdf and cdf
