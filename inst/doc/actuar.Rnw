...
The \pkg{actuar} project is a package of Actuarial Science functions
for \proglang{R}. Although various packages on CRAN provide functions
that may be of use to actuaries, \pkg{actuar} aims to serve as a
central location for more specifically actuarial functions and data
sets. The project was officially launched in 2005 and is under active
development.

The feature set of the package can be split in three main categories:
loss distributions modeling, risk theory (including ruin theory) and
credibility theory.
...
\emph{Chaire d'actuariat} (Actuarial Science Chair) of Université
Laval.
