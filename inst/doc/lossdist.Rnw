...
purposes. Package \pkg{actuar} offers many functions for loss
distributions modeling. The following subsections detail the following
\pkg{actuar} features:
\begin{enumerate}
...
\dots, Y^P_n$) from the random variable $Y^L$ ($Y^P$) to fit a
model on the unobservable random variable $X$. This requires
expressing the pdf or cdf of $Y^L$ ($Y^P$) in terms of the pdf or
...
possible. For comparison purpose, the following function contains no
