\documentclass{article}
  ...
  %\VignetteIndexEntry{Credibility theory features}
...
\item function \code{simul} to simulate data from compound
  hierarchical models;
...
Function \code{simul} simulates portfolios of data following compound
models of the form
\begin{equation}
...
Function \code{simul} is presented in the credibility theory section
...
\code{simul}. For the sake of completeness, we briefly outline this
method here.
...
Hence, function \code{simul} has four main arguments:
\begin{inparaenum}[1)]
...
of a data set with \code{simul} is achieved with:
<<echo=FALSE>>=
set.seed(3)
@
<<echo=TRUE>>=
...
\code{"portfolio"} containing all the individual claim amounts for each
...
\code{simul} can be impractical. For this reason, the \code{print}
