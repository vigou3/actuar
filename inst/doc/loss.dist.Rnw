\documentclass{article}
  ...
  \SweaveOpts{prefix.string=loss_dist}
...
coefficient in ruin theory; see the \texttt{"risk"} vignette.
...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
be the transition rates (or intensity) matrix of such a process and
let $(\mat{\pi}, \pi_{m + 1})$ be the initial probability
vector. Here, $\mat{T}$ is an $m \times m$ non-singular matrix with
$t_{ii} < 0$ for $i = 1, \dots, m$ and $t_{ij} \geq 0$ for $i \neq j$,
$\mat{t} = - \mat{T} \mat{e}$ and $\mat{e}$ is a column vector with
all components equal to 1. Then the cdf of the time until absorption
...
ruin; see \code{?ruin} and the \texttt{"risk"} vignette.
...
\begin{table}
  \begin{center}
    \caption{Coverage modifications for per-loss variable and
      per-payment variable as presented in \cite{LossModels2e}.}
    \label{tab:coverage}
    \begin{tabular}{lll}
      \toprule
