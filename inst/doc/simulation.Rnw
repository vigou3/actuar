...
Furthermore, actuaries often work with compound models of the form
\begin{equation}
  \label{eq:definition-S}
  S = C_1 + \dots + C_N,
\end{equation}
where $C_1, C_2, \dots$ are mutually independent and identically
distributed random variables each independent from $N$. Simulation of
data for such models is complicated by the need to always use the
correct parameters for each variate.
...
to avoid the incomplete calls to the random number generation
functions to be evaluated. Function \code{simul} builds and executes
the calls to the random generation functions from the top of the
...
@
  ...
  variates $X_{ijtu}$ in a matrix similar to those above, but with a
