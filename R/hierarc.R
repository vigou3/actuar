...
## Plots the graphs of premium lines and predictions, or this
## of the heterogeneity of the portfolio for a given level.
## Argument 'node' explicits the location in the tree.
## Example:
## node=c(2,3,4) <=> levels "contract", sect. 2, unit 3, contract 4.
plot.hierarc <- function(x, levels, node, add = FALSE,
                         type = c("predictions", "heterogeneity"),
                         main = NULL)
{
    ######## SANITY CHECKS ######
    ## check the class of the object, and 'levels'
    if (!inherits(x, "hierarc"))
        stop("use only with \"hierarc\" objects")
    if (is.null(levels))
        stop("'levels' can't be NULL in the hierarchical model")

    ## convert the character 'levels' argument into numeric
    level.names <- names(x$nodes)
    levels <- pmatch(levels, level.names)
    if (any(is.na(levels)) || !is.numeric(levels))
        stop("invalid level number")
    ## check if the 'node' corresponds to the right 'level'
    if (length(node) != levels)
        stop("bad specification of the node")
    ## number of levels
    ...
    if (type == "predictions")
    {
        xlabVect <- c(level.names, "data of contracts")
        ## Preparation of futur arguments of plot().
        ## Number of "brother" preceding ours at each level :
        ##   * 1st element : preceding left "sector" brothers
        ##   * 2nd element : preceding left "unit" brothers
        ##   * 3rd element : preceding left "contract" brothers
        nbLeftBrothers <- vector("integer", length = levels)
        nbLeftBrothers[1] <- nbLeftBrothersSect <- node[1] - 1
        ## Effective numbers in the list x$means
        realNo <- vector("integer", length = levels)
        realNo[1] <- realSectorNo <- node[1]

        ## suppose sector, unit and contract numbering start at '1'.
        if (levels == 1)   ## entity = 'sector'
        {
            firstObs <-
                if (nbLeftBrothersSect == 0)
                    ## subscript of the first observation
                    1
                else
                    1 + sum(x$nodes[[levels+1]][1:(realSectorNo-1)])
            ## number of observations
            abscisses <- x$nodes[[levels+1]][realSectorNo]
            ## range of observations
            obs <- x$means[[levels+2]][firstObs : (firstObs+x$nodes[[levels+1]][realSectorNo]-1)]
            collMean <- rep(x$means[[levels]], abscisses)
            indMean <- rep(x$means[[levels+1]][realSectorNo], abscisses)
            prediction <- rep(predict(x, levels = level.names[levels])[[1]][realSectorNo], abscisses)
            xlab <- xlabVect[2]
        }
        else
        {
            for (i in 2:levels)
            {
                if (nbLeftBrothers[i-1] != 0)
                {
                    nbLeftBrothers[i] <- sum(x$nodes[[i]][1:(realNo[i-1]-1)])
                    realNo[i] <- nbLeftBrothers[i] + node[i]
                }
                else
                {
                    nbLeftBrothers[i] <- node[i] - 1
                    realNo[i] <- node[i]
                }

                if (i != nlevels)   ## not yet at the lowest level
                {
                    firstObs <-
                        if ((node[i] == 1) && (nbLeftBrothers[i-1] == 0))
                            1
                        else
                            1 + sum(x$nodes[[i+1]][1:(realNo[i]-1)])
                    abscisses <- x$nodes[[i+1]][realNo[i]]
                    obs <- x$means[[i+2]][firstObs : (firstObs+x$nodes[[i+1]][realNo[i]]-1)]
                }
                else    ## at the lowest level
                {
                    abscisses <- ncol(x$ratios)
                    obs <- x$ratios[realNo[i], ]
                }
            }
            xlab <- xlabVect[i+1]
            collMean <- rep(x$means[[levels]][x$ordering[[levels]][realNo[i]]], abscisses)
            indMean <- rep(x$means[[levels+1]][realNo[i]], abscisses)
            prediction <- rep(predict(x, levels = level.names[levels])[[1]][realNo[i]], abscisses)
        }

        ## draw the data ( = observations)
        plot(1:abscisses, obs, type = "p", xlab = xlab, ylab = "premiums", ylim = c(0, max(collMean, indMean)),
             main = paste("Premiums in the hierarchical model, ", level.names[levels], " ", node[levels]))
        ## add the collective mean
        lines(1:abscisses, collMean, type = "l", col = "blue")
        ## add the individual mean
        lines(1:abscisses, indMean, type = "l", col = "red")
        ## add prediction
        lines(1:abscisses, prediction, type = "l", col = "green")
        ## add legend
        legend("bottomright", legend = c("observations", "collective premium", "individual premium", "credibility premium prediction"),
               text.width = strwidth("credibility premium prediction"), lty = c(-1,1,1,1), pch = c(21,-1,-1,-1), col = c("black","blue","red","green"),
               xjust = 1, yjust = 1, cex = 0.6)
    }
    else
    {
        ...
        ##  * 1st and 2nd element : ind. mean - 0.5*sqrt(within variance of the level entity)
        ##    (2 equal elements so as to hide the whiskers)
        ##  * 3rd element : ind. mean of the level entity
        ##  * 4th and 5th element : ind. mean + 0.5*sqrt(within variance of the level entity)
        stats <- matrix(0, nrow = 5, ncol = length(x$means[[levels+1]]))
        for (j in 1:length(x$means[[levels+1]]))
        {
            ## case of homogeneity of intercept
            stats[1, j] <- stats[2, j] <- x$means[[levels+1]][j] - 0.5*sqrt(x$unbiased[levels+1])
            stats[3, j] <- x$means[[levels+1]][j]
            stats[4, j] <- stats[5, j] <- x$means[[levels+1]][j] + 0.5*sqrt(x$unbiased[levels+1])
        }
        ## specify different attributes necessary to bxp()
        ## 'n' attribute = number of observations
        n <- numeric(length(x$means[[levels+1]]))
        ## 'confidence interval' attribute
        conf <- matrix(NA, nrow = 2, ncol = length(x$means[[levels+1]]))
        out <- numeric()          ## 'out' attribute
        group <- numeric()        ## 'group' attribute
        ## 'names' attribute
        names <- vector("character", length(x$means[[levels+1]]))
        for (i in 1:length(x$means[[levels+1]]))
            names[i] <- i

        bx.p <- list(stats = stats, n = n, conf = conf, out = out,
                     group = group, names = names)
        bxp(bx.p, show.names = TRUE, medcol = "red", medlwd = 7,
            main = paste("Heterogeneity of the level '", level.names[levels], "'"),
            boxwex = 0.001, staplewex = 30, xlab = level.names[levels],
