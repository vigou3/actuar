...
static SEXP dpqphtype2_1(SEXP sx, SEXP sa, SEXP sb, SEXP sI, double (*f)())
{
    ...
    double *x, *a, *b, *y;
    int i_1;
    Rboolean naflag = FALSE, nanflag = FALSE;
...
#define SETUP_DPQPHTYPE2						\
    ...
    y = REAL(sy);						\
								\
    if (ISNA (sa) || ISNA (b))  naflag  = TRUE;			\
    if (ISNAN(sa) || ISNAN(sb)) nanflag = TRUE
...
#define DPQPHTYPE2_1(A, FUN) dpq2_1(CAR(A), CADR(A), CADDR(A), CADDDR(A), FUN);
#define DPQPHTYPE2_2(A, FUN) dpq2_2(CAR(A), CADR(A), CADDR(A), CADDDR(A), CAD4R(A), FUN)

SEXP do_dpqphtype2(int code, SEXP args)
{
    switch (code)
    {
    case  1:  return DPQ2_1(args, dphtype);
    case  2:  return DPQ2_2(args, pphtype);
    case  3:  return DPQ2_1(args, mphtype);
    case  4:  return DPQ2_1(args, mgfphtype);
age
    default:
