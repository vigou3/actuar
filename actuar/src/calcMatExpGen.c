 ...
 *	x a vector of R^n.
 *
 *	We use an algorithm base on Octace's expm (file dMatrix.cc) and
 *  function R package Matrix's expm (file dgeMatrix.c) optimized for the 
 *	equation (1).
 *
...
SEXP calcMatExpGen (SEXP x, SEXP u, SEXP T, SEXP v)
{
	...
	int isUpperTriangular =0; //pseudo boolean: is T upper triangular ?
	int traceshiftT = 0;
	...
	*/
	...
	for(k = 0; k < lenX; k++)
	{
		...
		for(j = 7; j>=0; j--)
		{
			...
			// npp = T * npp + padec88[j] * T 
				// first compute matrix operation: work <- T %*% npp
			F77_CALL(dgemm) ("N", "N", &ncolT, &ncolT, &ncolT, &one, valT, &ncolT, npp, &ncolT, &zero, work, &ncolT);
				
				// second compute: work + padec88[j] * T
			for(i = 0; i < ncolTsqr; i++)
				npp[i] = work[i] + padec88[j] * valT[i];
			
			// dpp = T * dpp + (-1)^j * padec88[j] * T 
				// first compute matrix operation: work <- T %*% dpp
			F77_CALL(dgemm) ("N", "N", &ncolT, &ncolT, &ncolT, &one, valT, &ncolT, dpp, &ncolT, &zero, work, &ncolT);
			
				// second compute: work + (-1)^j * padec88[j] * T
			for(i = 0; i < ncolTsqr; i++)
		...
		// copy ncolTsqr character from npp to valT, i.e. T <- npp 
		...
		while(sqrpowscal --)
		{
			// compute matrix operation: work <- T %*% T
