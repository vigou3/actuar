 ...
 *  fonctions of the Burr distribution, to calculate raw moments and limited moments 
 *  of the random variable and to simulate random variates. See ../R/burr.R for details.
 *
...
double levburr(double d, double shape1, double scale, double shape2, double order, int give_log)
{
  double u;
    ...
    u = 1.0 / (1.0 + R_pow(d / scale, shape2));

    return R_pow(scale, order) * gammafn(1.0 + order / shape2) * gammafn(shape1 - order / shape2) * pbeta(1.0 - u, 1.0 + order / shape2, shape1 - order / shape2, 1, 0) / gammafn(shape1) + R_pow(d, order) * R_pow(u, shape1);
}
