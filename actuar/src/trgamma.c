...
double dtrgamma(double x, double shape1, double shape2, double scale, int give_log)
{
    ...
    return give_log ?
	dgamma(tmp1, shape1, tmp2, 1) + log(shape2) + (shape2 - 1.0) * log(x) :
...
double levtrgamma(double limit, double shape1, double shape2, double scale, double order, int give_log)
{
    ...
    return R_pow(scale, order) * gammafn(shape1 + tmp2) * pgamma(u, shape1 + tmp2, tmp1, 1, 0) / gammafn(shape1) + R_pow(limit, order) * pgamma(u, shape1, tmp1, 0, 0);
}
