...
double pinvtrgamma(double q, double shape1, double scale, double shape2, int lower_tail, int log_p)
{
  ...
  if (!R_FINITE(shape1) ||
    ...
    return (lower_tail ? R_D_exp(pgamma(tmp, shape1, scale, 0,1)):
	    R_D_exp(pgamma(tmp, shape1, scale, 1,1)));
}
...
double qinvtrgamma(double p, double shape1, double scale, double shape2, int lower_tail, int log_p)
{
  ...
  R_Q_P01_boundaries(p, 0, 1);
    ...
    return (lower_tail ? R_D_exp((-1.0 / shape2) * qgamma(1.0 - p, shape1, scale, 1, 1)) :
	    R_D_exp((-1.0 / shape2) * qgamma(1.0 - p, shape1, scale, 0, 1)));
}
...
double rinvtrgamma(double shape1, double scale, double shape2)
{
  if (!R_FINITE(shape1) ||
    ...
    return qgamma(unif_rand(), shape1, scale, 1, 0);
}
