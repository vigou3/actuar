...
severity.default <- function(x, bycol = FALSE, ...)
{
    ...
    frequencies <- array(sapply(x, fun), dim = dx)
...
### Method for objects of class 'simpf'. Two differences with the
### default method: 1) data can be grouped according to the levels of
### the portfolio; 2) data can be split by column (e.g. data from all
### years but the last and data from the last year).
